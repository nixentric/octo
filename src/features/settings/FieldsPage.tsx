        ...
        {fields.map((f, i) => {
          ...
          return (
            <li
              key={i}
              {...drag.rowProps(i)}
              className={cn('rounded-md border', drag.over === i && drag.from !== i && 'ring-2 ring-ring')}
            >
      ...
      <ul className="space-y-2">
        {options.map((o, i) => (
          <li
            key={i}
            {...drag.rowProps(i)}
            className={cn('flex items-center gap-2', drag.over === i && drag.from !== i && 'rounded ring-2 ring-ring')}
          >
            <span {...drag.handleProps} className="cursor-grab text-muted-foreground" aria-hidden><GripVertical className="size-4" /></span>
      ...
      <ul className="space-y-2">
        {children.map((child, i) => (
          <li
            key={i}
            {...drag.rowProps(i)}
            className={cn('flex flex-wrap items-center gap-2 rounded-md border p-2', drag.over === i && drag.from !== i && 'ring-2 ring-ring')}
          >
            <span {...drag.handleProps} className="cursor-grab text-muted-foreground" aria-hidden><GripVertical className="size-4" /></span>
